...
struct noArvoreBB {
  int valor ;
  struct noArvoreBB* esquerdo;
  struct noArvoreBB* direito;
  short altura ;
};
typedef struct noArvoreBB No;
...
void inicializa(No* raiz, int valor)
{
  raiz->valor = valor;
  raiz->esquerdo = NULL;
  raiz->direito=NULL;
...
int main(){
    ...
    int chave;
    No *aux, *raiz = (No *) calloc(1, sizeof(No));
    scanf ("%d", &chave) ;
    inicializa(raiz, chave);
    ...
    do{
        ...
        if (chave == -1)
            break ;
        aux = buscaBinaria(chave, raiz) ;
        if (aux)
        {
            raiz = remover (aux, chave) ;
            printf ("sim\n") ;
        }else
            raiz = inserir(raiz, chave) ;
            printf ("nao\n") ;
